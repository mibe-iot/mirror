...
function main() {
    ...
    const isAuthorized = bPrl.Characteristic(
        '00000001-94f3-4011-be53-6ac36bf22cf1',
        'isAuthorized',
        ['read', 'write-without-response']
    )
    const cmd = bPrl.Characteristic(
        '00000002-94f3-4011-be53-6ac36bf22cf1',
        'cmd',
        ['read', 'write', 'notify']
    )
    const bigData = bPrl.Characteristic(
        '00000003-94f3-4011-be53-6ac36bf22cf1',
        'bigData'
    )
    const myIpAddress = bPrl.Characteristic(
        '00000004-94f3-4011-be53-6ac36bf22cf1',
        'myIpAddress',
        ['encrypt-read']
    )
    const cpuTemp = bPrl.Characteristic(
        '00000006-94f3-4011-be53-6ac36bf22cf1',
        'cpuTemp',
        ['encrypt-read', 'notify']
    )
    ...
    isAuthorized.on('ReadValue', (device) => {
        console.log(
            bPrl.client.name +
                ', ' +
                device +
                ' has connected and checking if it is authorized'
        )
        ...
        switch (cmdNum) {
            ...
            case '3':
                ...
                bPrl.Adapter.logAllProperties()
                    ...
                    .catch((err) => {
                        console.error(
                            'Error calling Display adapter properties',
                            err
                        )
                    })
                break
            ...
            case '4':
                ...
                bPrl.Device.logAllProperties(bPrl.client.devicePath)
                    ...
                    .catch((err) => {
                        console.error(
                            'Error calling Connected device properties',
                            err
                        )
                    })
                break
    ...
    function updateAll() {
        ...
        cmd.setValue(
            '1=enable pairing, 2=disable pairing. 3=log adapter, 4=log connected device'
        )
        cpuTemp.setValue(getCpuTemp())
    }
}
...
bPrl.on('ConnectionChange', (connected) => {
    ...
    if (connected === true) {
        console.log(
            '--> ' +
                bleUserName +
                ' has connected to this server at ' +
                new Date().toLocaleTimeString()
        )
        if (bPrl.client.paired === false) {
            console.log(
                '--> ' + 'CAUTION: This BLE device is not authenticated.'
            )
        } else if (bPrl.client.paired === true) {
            console.log(
                '--> This device is paired and can read and write encrypted characteristics.'
            )
        } else {
            console.warn(
                "--> this device's paired state is not an expected value: " +
                    bPrl.client.paired
            )
        }
    } else {
        console.log(
            '<-- ' +
                bleUserName +
                ' has disconnected from this server at ' +
                new Date().toLocaleTimeString()
        )
...
function getCpuTemp() {
    ...
    try {
        ...
        var f = parseInt(cpuTempStr) * 0.001
        f = f * 1.8 + 32
